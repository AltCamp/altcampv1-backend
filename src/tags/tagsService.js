...
class TagsService {
   ...
   */
  async _createTag(tag, createdBy) {
    const tagExists = await this._tagExists(tag);
    if (tagExists) {
      return { _id: tagExists._id, name: tagExists.name };
    }

    const newTag = await Tag.create({ name: tag.toLowerCase(), createdBy });
   ...
   */
  async createTags(tags, createdBy) {
    const tagPromises = tags.map((tag) => this._createTag(tag, createdBy));
